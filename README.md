...
- 🔤 **Smart Handle & Case Formatting**: Automatic uppercase transformation by default, with preserved mixed case when an `@` handle prefix is typed (e.g., `@alexrivers`).
...
- 📲 **Smart Social Sharing & Desktop Alert**: Native Mobile Share Sheet integration sending your image file + caption directly to the X app via **"SHARE TO X · #FrameInGoa"**, plus instant desktop clipboard image copying with a prominent confirmation notification alert.
- 💡 **Integrated UX Tips Cards**: Contextual tips cards outside the input forms for both Builder Pass and PFP Frame modes guiding users on framing, handle formatting, custom title editing, and stack tag management.
